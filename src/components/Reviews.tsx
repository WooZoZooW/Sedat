import React from 'react';
import { useLanguage } from '../context/LanguageContext';
import styles from './Reviews.module.css';

const MOCK_REVIEWS = [
    {
        id: 1,
        author: "Max Mustermann",
        rating: 5,
        text: "Super Fahrschule! Ich habe meinen Führerschein beim ersten Mal bestanden.",
        time: "vor 2 Wochen"
    },
    {
        id: 2,
        author: "Anna Schmidt",
        rating: 5,
        text: "Sehr geduldige Fahrlehrer und angenehme Atmosphäre. Kann ich nur weiterempfehlen!",
        time: "vor 1 Monat"
    },
    {
        id: 3,
        author: "Ali Yilmaz",
        rating: 5,
        text: "Tolles Team, sehr moderne Autos. Hat viel Spaß gemacht!",
        time: "vor 3 Monaten"
    },
    {
        id: 4,
        author: "Elena Petrova",
        rating: 5,
        text: "Alles lief perfekt, schnelle Terminvergabe und gute Vorbereitung auf die Prüfung.",
        time: "vor 4 Monaten"
    }
];

const Reviews = () => {
    const { t } = useLanguage();

    const renderStars = (rating: number) => {
        return Array.from({ length: 5 }).map((_, i) => (
            <span key={i} className={i < rating ? styles.starFilled : styles.starEmpty}>
                ★
            </span>
        ));
    };

    return (
        <section className={`section-padding ${styles.reviewsSection}`} id="reviews">
            <div className={`container ${styles.container}`}>
                <h2 className="section-title">
                    {t('reviews.heading.part1')} <span className="accent-text">{t('reviews.heading.part2')}</span>
                </h2>

                <div className={styles.carouselWrapper}>
                    <div className={styles.carouselInfo}>
                        <div className={styles.googleIconWrapper}>
                            <svg className={styles.googleIcon} viewBox="0 0 24 24">
                                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.15v2.84C3.99 20.53 7.7 23 12 23z" />
                                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.15C1.43 8.55 1 10.22 1 12s.43 3.45 1.15 4.93l3.69-2.84z" />
                                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.15 7.07l3.69 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                            </svg>
                            <span className={styles.googleText}>{t('reviews.google')}</span>
                        </div>
                    </div>

                    <div className={styles.carousel}>
                        {MOCK_REVIEWS.map(review => (
                            <div key={review.id} className={`glass-card ${styles.reviewCard}`}>
                                <div className={styles.cardHeader}>
                                    <div className={styles.avatar}>
                                        {review.author.charAt(0)}
                                    </div>
                                    <div className={styles.authorInfo}>
                                        <h4>{review.author}</h4>
                                        <div className={styles.ratingInfo}>
                                            <div className={styles.stars}>
                                                {renderStars(review.rating)}
                                            </div>
                                            <span className={styles.time}>{review.time}</span>
                                        </div>
                                    </div>
                                </div>
                                <p className={styles.reviewText}>{review.text}</p>
                            </div>
                        ))}
                    </div>
                </div>
            </div>
        </section>
    );
};

export default Reviews;
